...
void checkRecordGRBCAT_1(const CatalogEntryGRBCAT& entry)
{
  ...
  ASSERT_FLOAT_EQ(39673.596851851798, entry.getTime().getMJD());
  ...
  ASSERT_FLOAT_EQ(0.0, entry.getCoodinates().getH());
  ASSERT_FLOAT_EQ(0.0, entry.getCoodinates().getV());
...
void checkRecordGRBCAT_10119(const CatalogEntryGRBCAT& entry)
{
  ...
  ASSERT_EQ(0, entry.getAltNames().size());
  ASSERT_FLOAT_EQ(53438.030104166697, entry.getTime().getMJD());
  ...
  ASSERT_FLOAT_EQ(182.62125, entry.getCoodinates().getH());
  ASSERT_FLOAT_EQ(77.617999999999995, entry.getCoodinates().getV());
